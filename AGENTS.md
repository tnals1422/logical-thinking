...
└── agents/          # 에이전트 정의 (orchestrator=프로세스 SoT, thinking, writing, problem-solving, expression, review, research)
struct-docs/
...
**Primary runtime: Grok Build** (`docs/archive/2026-07/grok-parent-as-orchestrator/` · decision stub: `docs/decisions/20260715-host-target-grok-only.md`)

```text
스킬 → Parent(메인)가 orchestrator.md 프로세스 인라인 → spawn(specialist only)
```

- **Parent-as-Orchestrator (L1)**: Brief · Spec · Prior · Review Gate · 저장 · memory 는 Parent가 수행. `subagent_type: "orchestrator"` 자식 스폰은 **primary 금지**.
- 설계·결정·KI·report (archived 묶음): `docs/archive/2026-07/grok-parent-as-orchestrator/`
- 각 전문 에이전트는 Minto Thinking 로직을 내장하여 독립 실행 가능.
- `orchestrator.md` 는 **프로세스 스펙(SoT)** (스폰 대상 아님).
- **네이티브 레이아웃**: skills/agents는 `.grok/` 에 위치 (Grok Build 공식 경로).
...
**Known Issue (Grok Build, 2026-07-15) — Closed**  
`orchestrator` 자식 스폰 실패 → **Parent-as-Orchestrator** 로 해결 (resolution D). V2–V4 검증 완료.  
런타임 전환 묶음 archive: `docs/archive/2026-07/grok-parent-as-orchestrator/` (decision · KI · design · report).  
CC entrypath: `docs/archive/2026-07/cc-entrypath-notes.md`. 운영 요약은 본 파일 + `struct-docs/usage/`.
...
- 템플릿/아키텍처 변경 시 `struct-docs/usage/*.md`, `.grok/agents/writing.md`, `templates/README.md` 동기화
