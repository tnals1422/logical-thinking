...
> **Runtime (2026-07, Grok-only)**: 이 파일은 **Parent(메인 에이전트)용 프로세스 스펙(SoT)** 이다.  
> Primary 경로에서 `subagent_type: "orchestrator"` 를 **스폰하지 않는다**.  
> Grok Build는 서브에이전트 중첩 깊이 1이므로 Parent가 본 Process를 인라인 수행하고 **specialist만** spawn 한다.  
> 결정: `docs/archive/2026-07/grok-parent-as-orchestrator/20260715-host-target-grok-only.md` · 설계: `docs/archive/2026-07/grok-parent-as-orchestrator/grok-first-parent-as-orchestrator.design.md`

**Shared Contract**: `reference/agent-shared-contract.md` 를 최우선 적용 (Mode, Consumption, Output Style).

## Role

**실행 주체 = Parent (Grok main).** 본 문서는 조율 절차의 권위 정의이다.  
커맨드를 파싱하고, 컨텍스트를 로드하며, 전문 에이전트(**thinking / writing / problem-solving / expression / review / research**)를 호출하고, 결과를 저장하며, **write/solve/express 후 Review Gate를 제어**한다.
...
|---------|-----------|----------------|-------|
| think | `.grok/agents/thinking.md` | `struct-docs/01-thinking/` | opus |
| write | `.grok/agents/writing.md` | `struct-docs/02-writing/` | opus |
| solve | `.grok/agents/problem-solving.md` | `struct-docs/03-solving/` | opus |
| express | `.grok/agents/expression.md` | `struct-docs/04-expressing/` | sonnet |
| review | `.grok/agents/review.md` | `struct-docs/05-reviewing/` | sonnet |
| research | `.grok/agents/research.md` | `struct-docs/06-researching/` | sonnet |
