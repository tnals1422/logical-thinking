...
```json
{
  ...
  "briefs": [],
  "maxItemsPerArray": 20,
  "userPreferences": { "reportTemplate": "report-default", "preferredFormat": "markdown", "defaultAudience": "expert" }
}
```
...
2. **Read**로 `deliverableTemplate` 존재 확인:
   ...
   - 없음 → `pending-phase2`, `fallbackTemplate: struct-docs/templates/report-default.md`
...
3. `logicPattern: auto` → schema §2.3 기본값 확정. `logicTemplate: struct-docs/templates/{pattern}.md`
...
8. **write 전용** `writingStages: skeleton → draft → audience-revised` — Phase 3 Writing Pipeline (solve/express에는 미포함)
9. `## Deliverable Spec` 블록 생성 (schema §2.1) — `meetingPurpose` / `eventPhase` / `writingStages`(write 시) 포함
...
- Frontmatter 포함 — write 시 `deliverableType`, `audience`, `draftStage: audience-revised`, `writingPipeline: w1-w2-w3` 추가
- write 시 Deliverable Spec.writingStages 순서로 W1→W2→W3 내부 실행 (writing.md Phase 3)
- Brief.audience·requestedActionRequired 반영; W3 Audience Pass 체크리스트 문서 하단 포함
...
- **Brief 복원** (Phase 4): `briefs` 배열에서 `linkedDocument`가 대상과 일치하는 항목 → `## Brief` 블록 재구성; 없으면 대상 frontmatter `deliverableType`·`audience`로 최소 Brief
...
- 프롬프트에 다음 포함:
  ...
  ## Output Requirements
  검증 유형에 맞는 Fidelity / Compliance / Deliverable Quality Report + Regeneration Directives + Review Data JSON 필수
  ```
...
- `fidelity`, `compliance`, `deliverableQuality`, `deliverableTiers`, `needs_regeneration`, `force_rework`, `critical_issues`, `directives_summary` 추출.
...
Review `deliverableQuality: fail` 또는 `force_rework` 진행 중이면 **Package 트리거 보류**.
