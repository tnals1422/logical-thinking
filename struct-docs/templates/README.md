...
## Migration Notes (Phase 5, 2026-06)

- deliverable 템플릿은 **high-level container + `(logic: xxx)` 앵커**만 유지.
- 패턴 내부 단계(기반구조, 상황 등)는 `patterns/*.md`에서 가져옴.
- 기존 산출물 재생성 시 W1에서 자동으로 패턴 단계 주입.
- Review D7은 이제 패턴 파일과 대조하여 검증.
- 구 deliverable 파일에 남아있던 상세 단계 복제는 Phase 2에서 제거됨.
- 신규 패턴 추가 시 `EMBEDDING-GUIDE.md`와 templates/README.md 동시 업데이트 필수.
